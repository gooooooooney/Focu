...
export const useEditorStore = create<EditorStore>()((set, get) => ({
    ...
    openFile: (projectId, fileId, { pinned }) => {
        ...
        const tabState = get().tabs.get(projectId) ?? defaultTabState

        const { openTabs, previewTabId } = tabState
        const isOpen = openTabs.includes(fileId)

        // Case 1: Opening as preview - replace existing preview tab or add new
        if (!isOpen && !pinned) {
        ...
        // Case 3: File is already open - set as active tab (and pin if double-clicked)
